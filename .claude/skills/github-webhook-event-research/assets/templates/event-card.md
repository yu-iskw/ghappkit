...
- `payload.action` value (or "absent" for `push`-style events): `<action | absent>`
...
- `<key-2>`
- `<...>`
- Notes: `<any subkeys that the receiver code reads, e.g. `pull_request.head.sha`>`
...
- Rationale: `<why this decision; cite missing fields, fast-changing payload, or upstream gap>`
- v1 typed coverage: `<yes|no>` per [`typed-context-mapping.md`](../../references/typed-context-mapping.md).

### Errors and edge cases

- Special handling notes: `<for example: pull_request.synchronize fires on every push to the head; check_run actions can fire many times per suite>`
